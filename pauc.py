...
def main():
    ...
    for m in ALL_MACHINE_TYPES:
        ...
        # store scores for combined pAUC
        src_scores_final = None
        tgt_scores_final = None
        ...
        for dom, trE, teE, labels in [
            ...
            # get scores for pAUC
            dm = DomainModel(**best["cfg"])
            dm.fit(trE)
            _, scores = dm.predict(teE)

            if dom == "source":
                src_scores_final = scores
            else:
        ...
        pauc = compute_pauc(combined_labels, combined_scores, max_fpr=0.1)

        print(f"  [combined] pAUC@0.1 = {pauc:.4f}")
